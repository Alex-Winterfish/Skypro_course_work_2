        ...
        try:
            ...
            for i in range(len(raw_list)):  # в этом цикле перебераем элементы
                for key, value in raw_list[
                    i
                ].items():  # в этом цикле перебераем словарь
            ...
            for vac in vac_list:
                if (
                    type(vac) is dict
                ):  # если элемент списка словарь, получаем начальный уровень зарплаты и обязаностей
                    for key, value in vac.items():
                        if key == "from":  # получем минимальный размер зарплаты
                            result_list.append(value)
                        elif (
                            key == "requirement"
                        ):  # получаем требования к специальности
                            result_list.append(value)
            ...
            for i in range(
                0, len(result_list), 5
            ):  # цикл для формирования списка вакансий
